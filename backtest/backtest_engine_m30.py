...
'''
    ...
    def print_realistic_results(self, execution_points, total_bars, pair_df: pd.DataFrame):
        ...
        print(f"   Processed Bars: {execution_points:,}")
        print(f"   Total Available Bars: {total_bars:,}")
        print(f"   Processing Rate: {execution_points/total_bars*100:.1f}% of bars")
        print(f"   Trading Costs: ZERO (Pure strategy performance)")
        ...
        print(f"   Lowest Balance: ${self.lowest_balance:,.2f}")
        print(f"   Max Drawdown: {results['max_drawdown_percent']:.2f}%")
        print(f"   Overall R:R Ratio: {results['overall_rr_ratio']:.2f}:1")
        # print(f"   Sharpe Ratio: {sharpe_ratio:.2f}") # Temporarily disabled, needs trades for std dev
...
if __name__ == "__main__":
    try:
        ...
        engine = BacktestEngineM30(
            ...
            target_pair="EUR/USD",
            start_balance=1000,
            is_autotuning=False, # Set to False for single backtest
            start_date=start_date_str,
            end_date=end_date_str
        )
        
        print(f"\n{'='*80}")
        print(f"🚀 STARTING SINGLE BACKTEST FOR EUR/USD with Combination 3")
        print(f"{'='*80}")
        ...
        print(f"\n{'='*80}")
        print(f"✅ SINGLE BACKTEST COMPLETED FOR EUR/USD")
        print(f"{'='*80}\n")
