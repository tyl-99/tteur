...
class GBPUSDSTRATEGY:
    """
    A Supply and Demand strategy for GBP/USD aiming for a high R:R.
    ...
    def __init__(self, target_pair="GBP/USD"):
        ...
        # --- 4H CABLE OPTIMIZATION PARAMETERS ---
        self.timeframe = "H4"
        self.zone_lookback = 120         # 🔄 Reduced from 300 (H4 lookback efficiency)
        self.base_max_candles = 4        # 🔄 Reduced from 5 (H4 bases)
        self.move_min_ratio = 1.6        # 🔄 Reduced from 2.0 (H4 moves are stronger)
        self.zone_width_max_pips = 45    # 🔄 Increased from 30 (wider H4 zones)
        self.pip_size = 0.0001
        
        # --- 4H CABLE WICK FILTER ---
        self.use_wick_filter = True  # Enable long wick filter
        self.min_wick_percentage = 0.4   # 🔄 Reduced from 0.5 (H4 wick adjustment)
        
        # --- 4H CABLE SETTINGS ---
        self.rr_target = 2.8            # 🔄 Increased from 2.0 (higher H4 targets)
        self.buffer_pct = 0.18          # 🔄 Increased from 0.15 (more conservative)
        self.atr_period = 14
        self.atr_min_mult = 0.4         # 🔄 Reduced from 0.5 (H4 adjustment)
        self.atr_max_mult = 1.3         # 🔄 Reduced from 1.5 (H4 control)
