...
class GBPUSDDemandStrategy:
    """
    Enhanced Supply and Demand strategy for GBP/USD with comprehensive volume analysis.
    ...
    7. GBP-specific volatility adjustments
    """

    def __init__(self, target_pair="GBP/USD"):
        self.target_pair = target_pair
        
        # Zone Detection Parameters (GBP pairs tend to be volatile)
        self.zone_lookback = 300
        self.base_max_candles = 5
        self.move_min_ratio = 2.2       # Higher threshold for GBP volatility
        self.zone_width_max_pips = 35   # Wider zones for GBP pairs
        self.pip_size = 0.0001
        
        # Volume Analysis Parameters
        self.volume_sma_period = 20           # Period for volume moving average
        self.volume_spike_threshold = 1.6     # Higher threshold for GBP pairs
        self.high_volume_threshold = 2.2      # 2.2x average = high volume for GBP
        self.volume_confirmation_weight = 0.3 # Weight of volume in zone strength
        self.min_volume_strength = 0.65       # Higher requirement for GBP volatility
    ...
    def _calculate_volume_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['price_volume_correlation'] = np.where(
            (df['price_change'] > 0) & (df['volume_ratio'] > 1.3), 1,  # Bullish with volume
            np.where((df['price_change'] < 0) & (df['volume_ratio'] > 1.3), -1, 0)  # Bearish with volume
        )
        
        # GBP-specific volume analysis
        df['gbp_volume_strength'] = np.where(
            df['volume_ratio'] > 2.0, 'very_strong',
            np.where(df['volume_ratio'] > 1.6, 'strong',
    ...
    def _calculate_zone_volume_strength(self, df: pd.DataFrame, base_start: int, base_end: int, impulse_idx: int) -> float:
        ...
        # 2. Volume spike confirmation (25% weight - higher for GBP)
        ...
        # 4. Volume trend confirmation (15% weight)
        if len(base_candles) >= 3:
            volume_trend_score = 0.15 if impulse_candle['volume_trend'] == 1 else 0.05
        else:
            volume_trend_score = 0.05
    ...
    def _is_strong_move_with_volume(self, base_candles: pd.DataFrame, impulse_candle: pd.Series) -> bool:
        ...
        # Price movement check (higher threshold for GBP)
        ...
        # Volume confirmation check (higher threshold for GBP)
                ...
                if 0 < zone_width_pips < self.zone_width_max_pips:
                    ...
                    if zone_type and volume_strength > best_volume_strength:
                        ...
                        best_zone = {
                            ...
                            'gbp_volume_strength': impulse_candle['gbp_volume_strength']
                        }
    ...
    def _check_volume_breakout_confirmation(self, df: pd.DataFrame, current_idx: int, zone: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Base volume requirement (adjusted for GBP)
        ...
        return {
            ...
            'gbp_volume_strength': current_candle['gbp_volume_strength']
        }
    ...
    def analyze_trade_signal(self, df: pd.DataFrame, pair: str) -> Dict[str, Any]:
        ...
        for zone in self.zones:
            ...
            if in_supply_zone or in_demand_zone:
                ...
                volume_multiplier = 0.5 + (zone['volume_strength'] * 0.5)  # 0.5x to 1.0x
                ...
                return {
                    ...
                    "volume_calculation": f"Base: {base_volume} × Volume Strength: {zone['volume_strength']:.2f} = {adjusted_volume:.2f} lots",
                    "meta": {
                        "zone_type": zone['type'],
    ...
    def get_volume_analysis_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'gbp_volume_profile': recent_data['gbp_volume_strength'].mode().iloc[0] if len(recent_data) > 0 else 'medium'
        } 