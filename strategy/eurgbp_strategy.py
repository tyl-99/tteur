...
class EURGBPSupplyDemandStrategy:
    ...
    def _find_technical_take_profit(self, df: pd.DataFrame, current_price: float, trade_direction: str, current_idx: int) -> Dict[str, Any]:
        ...
        if len(historical_data) < 50:
            ...
            fallback_pips = 40 if trade_direction == "BUY" else -40  # Conservative for EUR/GBP
            return {
        ...
        if trade_direction == "BUY":
            ...
            if targets:
                ...
                return {
                    'tp_price': target['price'] - (2 * self.pip_size),  # Place slightly below resistance
                    'tp_pips': (target['price'] - current_price) / self.pip_size - 2,
            ...
            if targets:
                ...
                return {
                    'tp_price': target['price'] + (2 * self.pip_size),  # Place slightly above support
                    'tp_pips': (current_price - target['price']) / self.pip_size - 2,
        ...
        target_pips = max(20, min(80, atr_pips * 1.8))  # 1.8x ATR, capped between 20-80 pips for EUR/GBP
