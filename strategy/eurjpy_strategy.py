...
class EURJPYSupplyDemandStrategy:
    """
    Enhanced Supply and Demand strategy for EUR/JPY with comprehensive volume analysis.
    ...
    7. JPY-specific volume characteristics
    """

    def __init__(self, target_pair="EUR/JPY"):
        self.target_pair = target_pair
        
        # Zone Detection Parameters (JPY pairs tend to be more volatile)
        self.zone_lookback = 300
        self.base_max_candles = 5
        self.move_min_ratio = 1.8       # Slightly lower for JPY volatility
        self.zone_width_max_pips = 40   # Wider zones for JPY pairs
        self.pip_size = 0.01            # JPY pip size
        
        # Volume Analysis Parameters
        self.volume_sma_period = 20           # Period for volume moving average
        self.volume_spike_threshold = 1.4     # Lower threshold for JPY pairs
        self.high_volume_threshold = 1.8      # 1.8x average = high volume for JPY
        self.volume_confirmation_weight = 0.3 # Weight of volume in zone strength
        self.min_volume_strength = 0.55       # Slightly lower for JPY volatility
    ...
    def _calculate_volume_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # JPY-specific volume analysis
        df['jpy_volume_strength'] = np.where(
    ...
    def _is_strong_move_with_volume(self, base_candles: pd.DataFrame, impulse_candle: pd.Series) -> bool:
        ...
        # Price movement check (adjusted for JPY volatility)
        ...
        # Volume confirmation check (lower threshold for JPY)
                ...
                if 0 < zone_width_pips < self.zone_width_max_pips:
                    ...
                    if zone_type and volume_strength > best_volume_strength:
                        ...
                        best_zone = {
                            ...
                            'zone_quality': 'high' if volume_strength > 0.75 else 'medium',
                            'jpy_volume_strength': impulse_candle['jpy_volume_strength']
                        }
    ...
    def _find_technical_take_profit(self, df: pd.DataFrame, current_price: float, trade_direction: str, current_idx: int) -> Dict[str, Any]:
        ...
        if len(historical_data) < 50:
            ...
            fallback_pips = 70 if trade_direction == "BUY" else -70  # Slightly wider for EUR/JPY
            return {
        ...
        target_pips = max(30, min(130, atr_pips * 2))  # 2x ATR, capped between 30-130 pips for EUR/JPY
    ...
    def _check_volume_breakout_confirmation(self, df: pd.DataFrame, current_idx: int, zone: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Base volume requirement (adjusted for JPY)
        ...
        return {
            ...
            'jpy_volume_strength': current_candle['jpy_volume_strength']
        }
            ...
            if in_supply_zone or in_demand_zone:
                ...
                return {
                    ...
                    "meta": {
                        ...
                        "confidence_level": "high" if zone['volume_strength'] > 0.75 else "medium",
                        "jpy_volume_strength": zone.get('jpy_volume_strength', 'medium'),
    ...
    def get_volume_analysis_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'jpy_volume_profile': recent_data['jpy_volume_strength'].mode().iloc[0] if len(recent_data) > 0 else 'medium'
        } 