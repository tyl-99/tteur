...
class USDJPYSTRATEGY:
    """
    USD/JPY Breakout Trading Strategy.
    ...
    def __init__(self,
                 range_lookback_candles: int = 12, # Changed from 8 to 12
                 min_range_pips: float = 40.0,
                 fixed_stop_loss_pips: float = 50.0,
                 take_profit_pips: float = 100.0 # Changed from take_profit_1_pips and take_profit_2_pips
                 ):
        ...
        self.take_profit_pips = take_profit_pips # Changed from take_profit_1_pips and take_profit_2_pips
    ...
    def analyze_trade_signal(self, df: pd.DataFrame, pair: str) -> Dict[str, Any]:
        """
        Very Simple Range Breakout Strategy for USD/JPY.
        """
        self.check_count += 1
        ...
        if current_bar['close'] > range_high:
            ...
            take_profit = entry_price + (self.take_profit_pips * self.pip_size) # Using TP2 for main target
            self.passed_count += 1
