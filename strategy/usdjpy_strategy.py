...
class USDJPYSupplyDemandStrategy:
    """
    Enhanced Supply and Demand strategy for USD/JPY with comprehensive volume analysis.
    ...
    7. USD/JPY-specific characteristics
    8. 1:5 Risk-to-Reward ratio
    """

    def __init__(self, target_pair="USD/JPY"):
        self.target_pair = target_pair
        
        # Zone Detection Parameters (USD/JPY characteristics)
        self.zone_lookback = 300
        self.base_max_candles = 5
        self.move_min_ratio = 1.9       # Balanced for USD/JPY
        ...
        self.volume_spike_threshold = 1.4     # Moderate threshold for USD/JPY
        self.high_volume_threshold = 1.9      # 1.9x average = high volume
        self.volume_confirmation_weight = 0.3 # Weight of volume in zone strength
        self.min_volume_strength = 0.6        # Standard requirement
    ...
    def _calculate_volume_metrics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # USD/JPY-specific volume analysis
        df['usdjpy_volume_strength'] = np.where(
            df['volume_ratio'] > 1.8, 'strong',
            np.where(df['volume_ratio'] > 1.4, 'medium', 'weak')
        )
                ...
                if 0 < zone_width_pips < self.zone_width_max_pips:
                    ...
                    if zone_type and volume_strength > best_volume_strength:
                        ...
                        best_zone = {
                            ...
                            'usdjpy_volume_strength': impulse_candle['usdjpy_volume_strength']
                        }
    ...
    def _find_technical_take_profit(self, df: pd.DataFrame, current_price: float, trade_direction: str, current_idx: int) -> Dict[str, Any]:
        ...
        if len(historical_data) < 50:
            ...
            fallback_pips = 60 if trade_direction == "BUY" else -60  # Standard for USD/JPY
            return {
        ...
        target_pips = max(25, min(120, atr_pips * 2))  # 2x ATR, capped between 25-120 pips for USD/JPY
    ...
    def _check_volume_breakout_confirmation(self, df: pd.DataFrame, current_idx: int, zone: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'usdjpy_volume_strength': current_candle['usdjpy_volume_strength']
        }

    def analyze_trade_signal(self, df: pd.DataFrame, pair: str) -> Dict[str, Any]:
        """Enhanced trade signal analysis with volume confirmation and 1:5 R:R"""
        ...
        for zone in self.zones:
            ...
            if in_supply_zone or in_demand_zone:
                ...
                # For USD/JPY, pip value = $10 * (USD/JPY rate / 100) per lot
                ...
                return {
                    ...
                    "meta": {
                        ...
                        "usdjpy_volume_strength": zone.get('usdjpy_volume_strength', 'medium'),
    ...
    def get_volume_analysis_summary(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        return {
            ...
            'usdjpy_volume_profile': recent_data['usdjpy_volume_strength'].mode().iloc[0] if len(recent_data) > 0 else 'medium'
        }