...
class EURUSDSTRATEGY:
    ...
    def _calculate_ema(self, df: pd.DataFrame, period: int, column: str = 'close') -> Optional[float]:
        """Compute EMA; return None if not enough data."""
        if len(df) < period:
            return None
    ...
    def _calculate_rsi(self, df: pd.DataFrame, period: int) -> Optional[float]:
        """Compute RSI; return None if not enough data."""
        if len(df) < period + 1:
            return None
    ...
    def analyze_trade_signal(self, df: pd.DataFrame, pair: str) -> Dict[str, Any]:
        ...
        if current_rsi is not None:
            # Determine rough bias based on latest zone for RSI filtering
            latest_zone = None
            if self.zones:
        ...
        if self.enable_news_sentiment_filter:
            ...
            # print(f"📰 News Sentiment: {news_sentiment}") # Optional: for debugging
            latest_zone = None
            if self.zones:
                latest_zone = self.zones[-1]
