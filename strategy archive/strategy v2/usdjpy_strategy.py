...
class USDJPYStrategy:
    def __init__(self, target_pair="USD/JPY"):
        ...
        self.zone_width_max_pips = 100   
        self.pip_size = 0.01
        
        # --- TREND FILTER OPTIMIZED FOR 30M DATA ---
        self.trend_ema_period = 34  # ~17 hours (better for 30M than 50)
        
        # --- PA FILTER SETTINGS (LESS STRICT) ---
        self.min_rejection_pips = 5  # Minimum rejection wick size for USD/JPY (reduced from 8)
        self.rsi_oversold = 30       # RSI level for oversold (relaxed from 25)
        self.rsi_overbought = 70     # RSI level for overbought (relaxed from 75)
        self.min_confluences = 1     # Minimum PA confluences required (Now testing stricter)
    ...
    def check_entry_signal(self, current_price: float, zone: Dict[str, Any], df: pd.DataFrame = None, current_index: int = None) -> Dict[str, Any]:
        """
    ...
    def analyze_trade_signal(self, df: pd.DataFrame, pair: str) -> Dict[str, Any]:
        """Strategy analysis with PA filtering - only trades when BOTH strategy + PA agree!"""
        ...
        # --- NEW: 30M-OPTIMIZED TREND CHECK ---
        ema = df['close'].rolling(self.trend_ema_period).mean().iloc[-1]
        ...
        for zone in self.zones:
            ...
            # === TREND FILTER (30M-aware) ===
            if (zone['type'] == 'demand' and not is_uptrend) or \
               (zone['type'] == 'supply' and is_uptrend):
            ...
            # ✅ STRATEGY WANTS TO TRADE - NOW CHECK WITH PA FILTER!
            zone['is_fresh'] = False  # Mark as used
            
            # Call enhanced check_entry_signal with DataFrame and index for PA analysis
            signal = self.check_entry_signal(
                current_price=current_price, 
                zone=zone, 
                df=df, 
                current_index=current_candle_index
            )
            
            # If PA approved, add trend metadata and return
            if signal.get("decision") != "NO TRADE":
                if "meta" not in signal:
                    signal["meta"] = {}
                signal["meta"]["trend"] = "UP" if is_uptrend else "DOWN"
                signal["meta"]["zone_type"] = zone['type']
                return signal
            # If PA rejected, continue to next zone
