...
class DetailedStrategyTester:
    ...
    def test_strategy_detailed(self, pair):
        ...
        try:
            ...
            for i in range(1, len(pair_df)):
                ...
                if active_trade is not None:
                    ...
                    if exit_price is not None:
                        ...
                        pnl = (exit_price - active_trade['entry_price']) * pnl_mult * 100000 - 1.0
            ...
            avg_win = sum(t['PnL_$'] for t in all_trades if t['PnL_$'] > 0) / wins if wins > 0 else 0
            avg_loss = sum(t['PnL_$'] for t in all_trades if t['PnL_$'] < 0) / losses if losses > 0 else 0
            profit_factor = abs(avg_win * wins / (avg_loss * losses)) if losses > 0 and avg_loss != 0 else float('inf')
            
            max_win = max(t['PnL_$'] for t in all_trades) if all_trades else 0
            max_loss = min(t['PnL_$'] for t in all_trades) if all_trades else 0
    ...
    def export_to_excel(self, pair, trades, result):
        ...
        filename = f"DETAILED_TRADES_{pair.replace('/', '_')}_{self.timestamp}.xlsx"
    ...
    def create_master_summary(self, all_results):
        ...
        filename = f"MASTER_STRATEGY_SUMMARY_{self.timestamp}.xlsx"
